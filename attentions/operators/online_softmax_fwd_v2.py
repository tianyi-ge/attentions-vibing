...
@triton.jit
def online_softmax_attn_fwd_kernel_v2(
...
def online_softmax_attention_forward_v2(
    ...
    *,
    ...
    if causal:
        raise NotImplementedError("online_softmax_fwd_v2 Triton kernel does not support causal masking yet.")
    if window_size:
        raise NotImplementedError("online_softmax_fwd_v2 Triton kernel does not support sliding-window masking yet.")
    if q.device.type != "cuda" or k.device.type != "cuda" or v.device.type != "cuda":
        raise NotImplementedError("online_softmax_fwd_v2 Triton kernel requires CUDA tensors.")
    ...
    online_softmax_attn_fwd_kernel_v2[grid](
        q, k, v, o,
